...
describe('sendErrorpro', () => {
  test('should send operational error response in production', () => {
    const err = new AppError('Operational error', 400)
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('in sendpro', err)
    expect(res.status).toHaveBeenCalledWith(400)
    expect(res.json).toHaveBeenCalledWith({
      status: 'fail',
      message: 'Operational error'
  ...
  test('should send generic error response for non-operational errors in production', () => {
    const err = new Error('Non-operational error')
